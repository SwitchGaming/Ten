...
// ═══════════════════════════════════════════════════════════════════
// MARK: - PREMIUM WIDGET 2: Overview (Large)
// ═══════════════════════════════════════════════════════════════════
...
struct OverviewWidget: Widget {
    ...
    var body: some WidgetConfiguration {
        ...
        .description("your rating, streak, and friends")
...
struct OverviewWidgetView: View {
    ...
    var body: some View {
        ...
        .widgetURL(URL(string: entry.isPremium ? "socialten://home" : "socialten://premium"))
    }
    
    private var premiumContent: some View {
        VStack(spacing: 16) {
            // Top row: Rating + Streak
            HStack(spacing: 12) {
                // Rating card
                ratingCard
            ...
            VStack(alignment: .leading, spacing: 12) {
                HStack {
                    ...
                    if entry.data.friends.count > 6 {
                        Text("+\(entry.data.friends.count - 6)")
                            .font(.system(size: 10, weight: .light))
                            .foregroundColor(theme.text3)
                    }
                }
                ...
                if entry.data.friends.isEmpty {
                    HStack {
                        ...
                        VStack(spacing: 6) {
                            Image(systemName: "person.2")
                                .font(.system(size: 24, weight: .light))
                                .foregroundColor(theme.text3.opacity(0.4))
                            Text("add friends to see their ratings")
                                .font(.system(size: 11, weight: .light))
                                .foregroundColor(theme.text3)
                        }
                    ...
                    // Friends grid - 2 rows of 3
                    VStack(spacing: 12) {
                        HStack(spacing: 20) {
                            ForEach(entry.data.friends.prefix(3)) { friend in
                                FriendBubble(friend: friend, defaultTheme: theme, size: 44, fontSize: 18)
                            }
                            if entry.data.friends.count < 3 {
                                ForEach(0..<(3 - entry.data.friends.count), id: \.self) { _ in
                        ...
                        if entry.data.friends.count > 3 {
                            HStack(spacing: 20) {
                                ForEach(entry.data.friends.dropFirst(3).prefix(3)) { friend in
                                    FriendBubble(friend: friend, defaultTheme: theme, size: 44, fontSize: 18)
                                }
                                if entry.data.friends.count < 6 {
                                    ForEach(0..<(6 - entry.data.friends.count), id: \.self) { _ in
                                        Spacer().frame(width: 44)
                                    }
                                }
                            }
    ...
    private var ratingCard: some View {
        ZStack {
            ...
            RoundedRectangle(cornerRadius: 16)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 16)
                        .stroke(theme.surface.opacity(0.3), lineWidth: 0.5)
                )
    ...
    private var streakCard: some View {
        let streak = entry.data.user?.currentStreak ?? 0
                ...
                ZStack {
                    if streak > 0 {
                        Image(systemName: "flame.fill")
                            .font(.system(size: 20))
                            .foregroundColor(streak >= 7 ? .orange : theme.glow)
                            .blur(radius: 10)
                            .opacity(0.5)
                    }
                    ...
                    Image(systemName: streak > 0 ? "flame.fill" : "flame")
                        .font(.system(size: 18))
                        .foregroundColor(streak > 0 ? (streak >= 7 ? .orange : theme.glow) : theme.text3.opacity(0.4))
                }
                
                Text("\(streak)")
                    .font(.system(size: 36, weight: .ultraLight))
                    .foregroundColor(streak > 0 ? theme.text1 : theme.text3.opacity(0.5))
...
#Preview("Overview", as: .systemLarge) {
    OverviewWidget()
} timeline: {
    TenWidgetEntry(date: Date(), data: .preview)
}

#Preview("Premium Locked", as: .systemLarge) {
    OverviewWidget()
} timeline: {
    TenWidgetEntry(date: Date(), data: WidgetData(
        user: WidgetUserData(displayName: "you", todayRating: 8, ratingTimestamp: Date(), currentStreak: 5, isPremium: false, themeId: "default"),
        friends: [],
        todaysPrompt: "test",
        latestPost: nil,
        theme: .default,
