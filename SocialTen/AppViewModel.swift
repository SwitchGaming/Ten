...
class AppViewModel {
    ...
    func loadSampleData() {
        ...
        // Sample vibes (isActive property used)
        let sampleVibe = Vibe(
            userId: "friend2",
            title: "Study?",
            timeDescription: "in 15 mins",
            location: "My place"
        )
        vibes = [sampleVibe]
    ...
    func getActiveVibes() -> [Vibe] {
        // only keep active vibes (Vibe uses isActive)
        vibes.filter { $0.isActive }.sorted { $0.timestamp > $1.timestamp }
    }
