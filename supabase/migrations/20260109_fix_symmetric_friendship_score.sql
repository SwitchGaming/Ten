-- Migration: Fix friendship score to be symmetric between both users
-- The issue was that friendship_weeks could differ because each direction
-- has a separate row with potentially different created_at timestamps.
-- Solution: Use the MINIMUM created_at from both friendship rows.
...
DECLARE
    ...
    total_score INT := 0;
BEGIN
    -- Get the EARLIEST friendship creation date from BOTH directions
    -- This ensures both users see the same friendship duration
    SELECT MIN(created_at) INTO friendship_created
    FROM friendships
...
-- Also fix the batch function
CREATE OR REPLACE FUNCTION calculate_batch_friendship_scores(current_user_id UUID)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    ...
    LOOP
        ...
        BEGIN
            ...
            SELECT MIN(created_at) INTO friendship_created
            FROM friendships
            WHERE (user_id = current_user_id AND friend_id = friend_record.friend_id)
               OR (user_id = friend_record.friend_id AND friend_id = current_user_id);
            ...
            FROM post_likes pl
            INNER JOIN posts p ON pl.post_id = p.id
            WHERE pl.user_id = current_user_id AND p.user_id = friend_record.friend_id;
            
            -- Count likes received (friend -> current user's posts)
            SELECT COUNT(*) INTO likes_received
            FROM post_likes pl
            INNER JOIN posts p ON pl.post_id = p.id
            WHERE pl.user_id = friend_record.friend_id AND p.user_id = current_user_id;
            
            -- Count replies given (current user -> friend's posts)
            SELECT COUNT(*) INTO replies_given
            FROM post_replies pr
            INNER JOIN posts p ON pr.post_id = p.id
            WHERE pr.user_id = current_user_id AND p.user_id = friend_record.friend_id;
            
            -- Count replies received (friend -> current user's posts)
            SELECT COUNT(*) INTO replies_received
            FROM post_replies pr
            INNER JOIN posts p ON pr.post_id = p.id
            WHERE pr.user_id = friend_record.friend_id AND p.user_id = current_user_id;
            
            -- Count vibe responses given (friend responded "yes" to current user's vibes)
            SELECT COUNT(*) INTO vibe_responses_given
            FROM vibe_responses vr
            INNER JOIN vibes v ON vr.vibe_id = v.id
            WHERE vr.user_id = friend_record.friend_id 
              AND v.user_id = current_user_id 
              AND vr.response = 'yes';
            ...
            INNER JOIN vibes v ON vr.vibe_id = v.id
            WHERE vr.user_id = current_user_id 
              AND v.user_id = friend_record.friend_id 
              AND vr.response = 'yes';
            
            -- Count matching rating days
            SELECT COUNT(*) INTO matching_rating_days
            FROM (
                SELECT DATE(rh1.date) as rating_date
                FROM rating_history rh1
                ...
                SELECT DATE(rh2.date) as rating_date
                FROM rating_history rh2
                WHERE rh2.user_id = friend_record.friend_id
            ) matching_dates;
            ...
            -- Build friend score JSON
            friend_score := json_build_object(
            ...
            friend_scores := array_append(friend_scores, friend_score);
        END;
    END LOOP;
    
    -- Return array of scores
    RETURN json_build_object('scores', friend_scores);
END;
$$;
